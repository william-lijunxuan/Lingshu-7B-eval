...
class SkinCAP(BaseDataset):
    def __init__(self,idx,model,dataset_path,output_path):
        self.model = model
        self.idx = idx
        self.output_path = output_path
        self.dataset_path = dataset_path if dataset_path else "joshuachou/SkinCAP"
        self.samples = []
        self.chunk_idx = int(os.environ.get("chunk_idx",0))
        self.num_chunks = int(os.environ.get("num_chunks",1))
        self.eval_local_datasets_flag = bool(strtobool(os.environ.get("eval_local_datasets_flag",True)))
        self.eval_local_datasets_file = str(os.environ.get("eval_local_datasets_file", "/mnt/d/skinalor/dataset/skin/SkinCAP/SkinCAP_20250712_013256.json"))


    def load_data(self):

        image_num =int(os.environ.get("image_num", 0))
        # load local datasets
    ...
    def construct_multi_image_rag_prompt(self,sample: dict) -> dict:

        type=sample["question_type"]
        description=sample["description"]
        if "close_end_QA"==type:
            prompt_text = "What is the name of the disease shown in the image?"+" Here is the description of the picture. "+description
            # prompt_text = get_close_ended_prompt("What is the name of the disease shown in the image?")
            # prompt_text = prompt_text+" Here is the description of the picture. "+description
            ...
            """
        primary_img_path = os.path.join("/mnt/d/skinalor/dataset/skin/SkinCAP/skincap", sample["image_name"])
        ...
        #
        #     try:
        #         primary_image_object = Image.open(primary_img_path)
        #         all_images.append(primary_image_object)
        #     except FileNotFoundError:
        #         print(f"Error: The primary image file {primary_img_path} was not found.")
        #         return None
        #     prompt_text = "The primary case is shown in <image_1>. " + prompt_text
        #
        #     # 4. Use the primary image to retrieve similar cases (text and image names)
        #     # retrieved_cases = retriever.retrieve_similar_cases(image, "Retrieve similar cases.",1)
        #     retrieved_cases = retriever.retrieve_similar_cases(image, sample["description"],1)
        #
        #     # 5. Augment the prompt with retrieved text AND load retrieved images
        #     metadatas = retrieved_cases.get('metadatas', [[]])[0]
        #
        #     if not metadatas:
        #         print("No similar cases were found in the database.\n")
        #     else:
        #         prompt_text += "\n\nFor your reference, here are some similar cases from a medical database:\n"
        #
        #         # The next image will be <image_2>, <image_3>, etc.
        #         image_counter = 2
        #
        #         for i, meta in enumerate(metadatas):
        #             retrieved_img_name = meta.get("image_filename")
        #             if not retrieved_img_name:
        #                 # Skip this case if it doesn't have an image name in its metadata
        #                 print(f"Warning: Retrieved case {i + 1} is missing 'image_name' in metadata. Skipping.")
        #                 continue
        #
        #             retrieved_img_path = os.path.join("/mnt/d/skinalor/dataset/skin/SkinCAP/skincap", retrieved_img_name)
        #             try:
        #                 # Load the retrieved image
        #                 retrieved_image_object = Image.open(retrieved_img_path)
        #
        #                 # Add the retrieved image to our list of images
        #                 all_images.append(retrieved_image_object)
        #
        #                 # Add the text description to the prompt, referring to the new image's position
        #                 prompt_text += f"\n--- Reference Case {i + 1} (see <image_{image_counter}>) ---\n"
        ...
        #
        #             except FileNotFoundError:
        #                 print(f"Warning: The retrieved image file {retrieved_img_path} was not found. Skipping case.")
        #                 continue
        #
        #     # 6. Create the final message dictionary for the multi-image model
        #     final_message = {
        #         "prompt": prompt_text,
        #         "images": all_images  # This list contains the original image + all retrieved images
        #     }
        #     sample["messages"] = final_message
        # else:
    ...
    def construct_messages(self,sample):
        ...
        if "close_end_QA"==type:
            # prompt_text = get_close_ended_prompt("What is the name of the disease shown in the image?")
            prompt_text = "What is the name of the disease shown in the image?"
        elif "open_end_QA"==type:
            prompt_text = """You are a board‐certified dermatology AI specialist. A patient has just uploaded an image of a skin lesion. Carefully examine the lesion’s visual features—color, shape, borders, surface texture, and anatomic location—and then compose a single, fully descriptive diagnostic sentence in English. Mirror the expert style by:
            1. Opening with a concise description of the key visual finding (e.g. “The red, smooth, exophytic nodule with a slightly narrowed base…”).
            2. Stating the most likely diagnosis (e.g. “…may indicate squamous cell carcinoma.”).
        ...
        img_path = os.path.join("/mnt/d/skinalor/dataset/skin/SkinCAP/skincap", sample["image_name"])
